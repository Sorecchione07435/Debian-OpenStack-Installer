...
crudini --set $conf DEFAULT glance_api_servers http://$HOST_IP:9292
...
crudini --set $conf keystone_authtoken username cinder
crudini --set $conf keystone_authtoken password $SERVICE_PASSWORD

crudini --set $conf oslo_concurrency lock_path /var/lib/cinder/tmp
